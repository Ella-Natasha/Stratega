...
The run methods are called *once* at the start of the game and should not retirm before the game is over.
The game communicator object can be used to query elements from the game state, create copies of it and to indicate what actions must be executed in the real game.
Additionally, the agent receives the forward model object for simulating the game. Depending on which YAML-file was passed to Stratega, the forward model will behave differently. 
...
#. To use the agent in a game, modify the list of agents in its configuration file in './gameConfigs' to include MyAgentName
